...
vector<vector<Vector>> ToneReproduction::AdaptiveLogarithmScale(vector<vector<Vector>> radiances) {
	// STEP 1: Incoming in @param radiances
	vector<vector<double>> luminances;
	...
	for (int i = 0; i < radiances.size(); i++) {
		...
		for (int j = 0; j < radiances[i].size(); j++) {
			...
			if (luminance > Lwmax) Lwmax = luminance;
	...
	for (int i = 0; i < radiances.size(); i++) {
		...
		for (int j = 0; j < radiances[i].size(); j++) {
			...
			double Lw = (cur.vec[0] * 0.27 + cur.vec[1] * 0.67 + cur.vec[2] * 0.06) / Lwa;
			double bPart = log(b) / log(0.5);
			double lowerInnards = pow((Lw / Lwmax), bPart) * 8;
			double Ld = log(Lw + 1.0) / (log10(Lwmax + 1.0) * log(lowerInnards + 2.0)); 
			cur = cur.scale(Ld);
